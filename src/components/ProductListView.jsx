  ...
  return (
    <div className='space-y-4 mt-2 rounded-md'>
        <div className='bg-gray-100 flex gap-7 items-center p-2 rounded-md '>
            <img src={product.thumbnail} alt={product.title} className='md:h-60 md:w-60 w-25 h-25 rounded-md cursor-pointer ' onClick={()=>navigate(`/products/${product.id}`)}/>
            <div className="space-y-2">
           <h1 className='font-bold md:text-xl text-lg line-clamp-3 hover:text-red-400 md:w-full max-w-[220px]'>{product.title}</h1>
           <p className='font-semibold flex items-center text-sm md:text-lg'>$<span className='md:text-4xl text-3xl'>{product.price}</span> ({product.discountPercentage
})% off</p>
